...
def _cv_bytes() -> bytes:
    ...
    document.add_paragraph(
        "Senior Software Engineer at TechCorp (2019-2024): Led development of microservices architecture using Python and Docker."
    )
    document.add_paragraph(
        "Software Engineer at StartupXYZ (2017-2019): Developed RESTful APIs using FastAPI and PostgreSQL."
    )
    document.add_paragraph("Formation")
    document.add_paragraph(
        "Bachelor of Science in Computer Science, University of State (2017)"
    )
    document.add_paragraph("Compétences")
    document.add_paragraph(
        "Python, FastAPI, Docker, Kubernetes, PostgreSQL, AWS, Git, CI/CD"
    )
    buffer = io.BytesIO()
...
def test_full_flow_register_login_diagnose_list_delete(client):
    ...
    login = client.post(
        "/auth/login", data={"username": "flow@example.com", "password": "s3cret!1"}
    )
