...
def _job(title: str = "Développeur en alternance") -> dict:
    return {
        ...
        "apply": {
            "url": "https://labonnealternance.apprentissage.beta.gouv.fr/offres/123"
        },
    }
...
def test_search_returns_normalized_listings():
    respx.get(SEARCH_URL).mock(
        return_value=httpx.Response(200, json={"jobs": [_job()]})
    )
    ...
    assert (
        listings[0].url
        == "https://labonnealternance.apprentissage.beta.gouv.fr/offres/123"
    )
...
def test_search_filters_out_jobs_not_matching_keyword():
    respx.get(SEARCH_URL).mock(
        return_value=httpx.Response(
            200, json={"jobs": [_job("Développeur"), _job("Comptable")]}
        )
    )
...
def test_search_raises_on_response_wrong_field_type():
    respx.get(SEARCH_URL).mock(
        return_value=httpx.Response(
            ...
            json={
                "jobs": [
                    {"offer": {"title": "Développeur"}, "workplace": "not an object"}
                ]
            },
        )
    )
